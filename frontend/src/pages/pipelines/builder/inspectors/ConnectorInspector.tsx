...
export default function ConnectorInspector({ config, onChange, readOnly = false }: { config: Record<string, unknown>; onChange: (key: string, value: unknown) => void; readOnly?: boolean }) {
  ...
  const onDrop = useCallback((accepted: File[]) => { const n = [...dragFiles, ...accepted]; setDragFiles(n); onChange('files', n.map(f => ({ name: f.name, size: f.size }))) }, [dragFiles, onChange])
  const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, multiple: true })

  const hasStoredFiles = storedFiles.length > 0 || dragFiles.length > 0
    ...
    return (
      <div className="space-y-3">
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs">
          ...
          {sourceType === 'file' && dragFiles.map((f, i) => (
            <p key={`d${i}`} className="text-blue-500">📄 {f.name} {formatSize(f.size)}</p>
          ))}
      ...
      {sourceType === 'file' && (
        <div>
          ...
          <div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors ${isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-400'}`}>
            ...
            {isDragActive ? <p className="text-xs text-blue-500 font-medium">松开以添加文件</p> : <p className="text-xs text-gray-500">拖拽文件到此处，或<span className="underline ml-0.5">点击选择</span></p>}
            <p className="text-[10px] text-gray-400 mt-1">支持 CSV/XLSX/JSON/PDF/DOCX 等，可批量多选</p>
          </div>
          {hasStoredFiles && (<div className="mt-2 space-y-1">{[...storedFiles, ...dragFiles].map((f: any, i: number) => (
            <div key={i} className="flex items-center gap-2 text-xs bg-gray-50 rounded px-2 py-1.5">
              ...
              <button onClick={() => { const all = [...storedFiles, ...dragFiles].filter((_: any, j: number) => j !== i); setDragFiles([]); onChange('files', all) }} className="text-gray-400 hover:text-red-500"><X size={11} /></button>
            </div>
          ))}</div>)}
        </div>
      )}
