...
> **核心参考**: Palantir Foundry 的五阶段数据集成链路，结合开源技术栈落地。
...
| **④ 写入 Neo4j** | Entity Type → Node Label；每行数据 → Node；每条关系 → Relationship | - |
| **⑤ 写入 ChromaDB** | 实体描述+属性 → 向量嵌入，用于后续语义搜索 | - |
...
| **Logic** | 沿用 v1 `LogicTab.tsx` | 保留逻辑规则列表 CRUD；**内置搜索栏**（关键词过滤 + 关联实体筛选） |
| **Actions** | 沿用 v1 `ActionsTab.tsx` | 保留动作列表 CRUD；**内置搜索栏**（关键词过滤 + 关联实体/逻辑筛选） |
...
| ⑧⑩ | Mapping | LLM 辅助生成映射建议（Entity Type / Property / Relation），用户确认后执行。进度条展示 5 个阶段百分比 |
